...
simulate_data <- function(n = 1000, sigma = 1, 
  ...
  # Add small noise to make sure invertible
  # See here https://bookdown.org/rbg/surrogates/chap5.html#chap5gppost
  eps <- sqrt(.Machine$double.eps)
  cov_mat <- exp(-d^2 / sigma) + diag(eps, ncol(d))
  ...
  # Generate outcome, include or exclude an interaction term 
  if (interaction) {
    outcome <- s^2 - 3 * sqrt(abs(v)) +  w*(0.1 - s + 0.1*v) +
      0.01*w^3 + rnorm(n, sd = sqrt(10)) + 10
  } else {
    outcome <- s^2 - 3 * sqrt(abs(v)) +  w*(0.1) +
      0.01*w^3 + rnorm(n, sd = sqrt(10)) + 10
  }
...
# Function to fit models to the data
fit_models <- function(dat, n_core, spatial_hidden, w_values) {
  ...
  gam_fit <- mgcv::gam(reformulate(c(cov_names, "s(w)"), response = "outcome"), data = dat)
  ...
  gam_ent_weights <- mgcv::gam(reformulate(c(cov_names, "s(w)"), response = "outcome"),
...
fit_gpcerf <- function(dat, n_core, cov_names,
  ...
  if (nn) {
    ...
    # Else fit full GP model, currently only the gaussian kernel
    # Change alpha and beta's at some point too
    # beta_to_alpha <- seq(0, 1/2, length.out = 6)
    # scale <- c(1, 2, 3, 4, 5)
...
predict_outcome <- function(dat, lm_fit, gam_fit, lm_ent_weights,
                            gam_ent_weights, w_values, interaction) {
...
calculate_metrics <- function(dat, lm_fit, gam_fit, lm_ent_weights,
  ...
  data_prediction <- predict_outcome(dat, lm_fit, gam_fit, lm_ent_weights, gam_ent_weights,
                                     w_values, interaction = interaction)
