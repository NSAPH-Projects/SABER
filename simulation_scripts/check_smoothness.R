...
gps_m_normal <- GPCERF::estimate_gps(cov_mt = data_sim_normal[,-(1:2)],
                                     w_all = data_sim_normal$treat,
                                     sl_lib = c("SL.xgboost"),
                                     dnorm_log = FALSE)

gps_m_t <- GPCERF::estimate_gps(cov_mt = data_sim_t[,-(1:2)],
...
params_lst <- list(alpha = 10 ^ seq(0, 2, length.out = 5),
...
gp_res_normal2 <- estimate_cerf_gp(data_sim_normal,
                                  w_all,
                                  gps_m_normal,
                                  params = list(alpha = c(5),
                                                beta = c(10),
                                                g_sigma = 0.01,
                                                tune_app = "all"),
                                  nthread = 40)
...
gp_res_t <- estimate_cerf_gp(data_sim_t,
                             ...
                             params = list(alpha = c(5),
                                           beta = 10,
                                           g_sigma = seq(0, 0.25, length.out = 10),
                                           tune_app = "all"),
                             nthread = 10)

# When we increase the sigma we get a more flexible model that covers the curve but is less acurate. 
# Uncertainty seems to be quantified better in this case though. 
gp_res_t <- estimate_cerf_gp(data_sim_t,
                             w_all,
                             gps_m_t,
                             params = list(alpha = c(5),
